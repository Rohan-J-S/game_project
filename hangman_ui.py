...
user_text = text()
word = text()
word.y,word.text,word.x = word.y-30,main_module_1.word(),word.x-5
run[1],run[2] = len(word.text) - word.text.count(' '),set()
time_left = text()
    ...
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_BACKSPACE:
                ...
                run = main_module_1.game(word.text,user_text.text,run[1],run[2],run[3])
            else:
